...
export default function Post({ postData }: PostProps) {
  ...
  const handleReport = async () => {
    ...
    if (!window.confirm("Bu gönderiyi şikayet etmek istiyor musunuz?")) return;
    ...
    try {
      ...
      alert("Şikayet başarısız: " + err.message);
    }
  };
                ...
                .map((word, index) => {
                  ...
                  return (
                    <span
                      ...
                      onMouseEnter={() => !isHashtag && translateWordWithInput(word, index)}
                      onMouseLeave={() => {
                        setActiveWordIndex(null);
                        setUserInput("");
                        setFeedback(null);
            ...
            {youTubeId ? (
              ...
              <div className="mt-4 max-w-full">
                {postData.media_type === "video" ? (
                  ...
                  <Image src={postData.media_url} alt="Post media" width={800} height={600} className="w-full max-h-[400px] rounded-lg object-contain shadow-lg" />
                )}
              </div>
            ) : null}
