...
export default function Comments({ postId }: CommentsProps) {
  ...
  const handleReport = async (commentId: number) => {
    ...
    try {
      const res = await fetch("/api/reports", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ comment_id: commentId, reason: "Uygunsuz yorum" }),
      });
      if (res.ok) alert("Yorum şikayet edildi.");
    } catch (err) {
      console.error("Şikayet hatası:", err);
      alert("Şikayet başarısız.");
    }
  };

  const handleReply = async (parentId: number) => {
    const reply = prompt("Yanıtınızı yazın:");
    if (!auth?.user || !reply?.trim()) return;
    try {
      const res = await fetch(`/api/posts/${postId}/comment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            <div className="w-full">
              <div className="flex justify-between items-center">
                ...
                <div className="relative">
                  <button
                    onClick={() =>
                      setActiveDropdown((prev) => (prev === c.id ? null : c.id))
                    }
                    className="text-gray-400 hover:text-orange-400 text-xl px-2"
                  >
                    ⋯
                  </button>
                  {activeDropdown === c.id && (
                    <div className="absolute right-0 mt-2 w-32 bg-gray-900 border border-gray-700 rounded-lg shadow z-50">
                      {isOwner ? (
                        <button
  ...
  return (
    <div className="mt-4 p-4 bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg shadow-md border border-gray-600">
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
